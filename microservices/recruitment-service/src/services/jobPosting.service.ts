...
export class JobPostingService {
  ...
  async createJobPosting(data: CreateJobPostingInput, createdById?: string): Promise<JobPostingWithDetails> {
    try {
      // Validate that closing date is after posted date if both are provided
      if (data.postedDate && data.closingDate && data.closingDate <= data.postedDate) {
        throw new AppError('Closing date must be after posted date', 400);
      }
      ...
      throw new AppError('Failed to create job posting', 500, error);
    }
  }

  async getJobPostings(query: JobPostingQueryInput): Promise<PaginatedResponse<JobPostingWithDetails>> {
    try {
      const {
        page = DEFAULT_PAGE,
        limit = DEFAULT_LIMIT,
      ...
      throw new AppError('Failed to fetch job postings', 500, error);
    }
  }

  async getJobPostingById(id: string): Promise<JobPostingWithDetails> {
    try {
      ...
      if (error instanceof AppError) throw error;
      throw new AppError('Failed to fetch job posting', 500, error);
    }
  }

  async updateJobPosting(id: string, data: UpdateJobPostingInput, updatedById?: string): Promise<JobPostingWithDetails> {
    try {
      ...
      if (error instanceof AppError) throw error;
      throw new AppError('Failed to update job posting', 500, error);
    }
  }
  ...
  async deleteJobPosting(id: string): Promise<void> {
    try {
      ...
      if (existingJobPosting._count.applications > 0) {
        throw new AppError('Cannot delete job posting with existing applications. Please archive it instead.', 400);
      }
      ...
      if (error instanceof AppError) throw error;
      throw new AppError('Failed to delete job posting', 500, error);
    }
  }

  async approveJobPosting(id: string, data: ApproveJobPostingInput, approvedById?: string): Promise<JobPostingWithDetails> {
    try {
      const existingJobPosting = await this.prisma.jobPosting.findUnique({
        where: { id },
      });
      ...
      if (error instanceof AppError) throw error;
      throw new AppError('Failed to approve/disapprove job posting', 500, error);
    }
  }

  async bulkUpdateJobPostingStatus(data: BulkUpdateJobPostingStatusInput, updatedById?: string): Promise<BulkOperationResult> {
    try {
      ...
      for (const id of jobPostingIds) {
        try {
          ...
          failed++;
          errors.push(`Failed to update job posting ${id}: ${(error as any).message}`);
        }
      }
      ...
      throw new AppError('Failed to bulk update job posting status', 500, error);
    }
  }
  ...
  async getJobPostingStats(): Promise<any> {
    try {
      ...
      throw new AppError('Failed to fetch job posting statistics', 500, error);
    }
  }
